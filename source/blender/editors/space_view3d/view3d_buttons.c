...
static int v3d_mesh_constraints_buts(BMDim *edm, int totdim, uiBlock *block,int *constraints, int yi, int buth, int but_margin) {

	// Enable / Disable according setting
	bool all = totdim > 1;
	bool none = ((*constraints & DIM_CONSTRAINT_OVERRIDE) == 0);

	UI_block_lock_set(block,none || !(all || valid_constraint_setting (edm, DIM_PLANE_CONSTRAINT)), "Valid Contraint");
	...
	UI_block_lock_set(block,none || !(all || valid_constraint_setting (edm, DIM_AXIS_CONSTRAINT)), "Valid Contraint");
	...
	UI_block_lock_set(block,none || !(all || valid_constraint_setting (edm, DIM_ALLOW_SLIDE_CONSTRAINT)), "Valid Contraint");
				...
				BM_ITER_MESH (erf, &iter, em->bm, BM_REFERENCES_OF_MESH) {
					if (erf->type == search_arg->reference_type) {
						if (BLI_strcasestr(erf->name, str)) {
							...
							BLI_strncpy(name, erf->name, 256);
							if (false == UI_search_item_add(items, name, erf, 0))
								break;
						}
					}
				}
	...
	if (block) { /* buttons */
		...
		if (totdim) {
			...
			if (totdim == 1) {
				...
				if (edm_sel->mdim->dim_type == DIM_TYPE_ANGLE_3P){
			...
			yi= v3d_mesh_constraints_buts(edm_sel, totdim, block,&tfp->dim_constraints, yi, buth, but_margin);
...
static void do_view3d_mesh_dimensions_buttons(bContext *C, void *UNUSED(index), int event)
{
	...
	switch (event) {
		...
		case B_OBJECTPANELMEDIAN:
			if (ob) {
				v3d_mesh_dimensions_buts(scene, NULL, v3d, ob, 1.0);
				DEG_id_tag_update(&ob->id, OB_RECALC_DATA);
			}
			break;
	}
...
static void view3d_panel_mesh_dimensions(const bContext *C, Panel *pa)
{
	...
	UI_block_func_handle_set(block, do_view3d_mesh_dimensions_buttons, NULL);
...
void view3d_buttons_register(ARegionType *art)
{
	...
	pt = MEM_callocN(sizeof(PanelType), "spacetype mesh dimensions panel object");
	strcpy(pt->idname, "VIEW3D_PT_mesh_dimensions");
