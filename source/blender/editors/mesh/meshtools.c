...
static void join_mesh_single(
        ...
        MVert **mvert_pp, MEdge **medge_pp, MLoop **mloop_pp, MPoly **mpoly_pp, MDim **mdim_pp,
        ...
        int *vertofs, int *edgeofs, int *loopofs, int *polyofs, int *dimofs)
{
	int a, b;
	...
	if (me->totdim) {
		for (a = 0; a < me->totdim; a++) {
			...
			for (int i=0;i<me->mdim[a]->totverts;i++) {
...
int join_mesh_exec(bContext *C, wmOperator *op)
{
	...
	join_mesh_single(
	            bmain, scene,
	            ob, ob_base, imat,
	            &mvert, &medge, &mloop, &mpoly, mdim,
	            &vdata, &edata, &ldata, &pdata,
	            totvert, totedge, totloop, totpoly,
	            key, nkey,
	            matar, matmap, totcol,
	            &vertofs, &edgeofs, &loopofs, &polyofs, &dimofs);
