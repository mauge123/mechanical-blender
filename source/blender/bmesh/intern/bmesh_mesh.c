...
static void bm_mempool_init_ex(
		...
		BLI_mempool **r_vpool, BLI_mempool **r_epool, BLI_mempool **r_lpool, BLI_mempool **r_fpool, BLI_mempool **r_dpool,  BLI_mempool **r_ppool,  BLI_mempool **r_gpool)
{
	size_t vert_size, edge_size, loop_size, face_size, dim_size, ref_size, geom_size;
...
#ifdef WITH_MECHANICAL_MESH_DIMENSIONS
		dim_size  = sizeof(BMDim_OFlag);
#endif
#ifdef WITH_MECHANICAL_MESH_REFERENCE_OBJECTS
		ref_size = sizeof (BMReference_OFlag);

#endif
#ifdef WITH_MECHANICAL_GEOMETRY
		geom_size = sizeof(BMGeom);
#endif
	}
	else {
...
#ifdef WITH_MECHANICAL_MESH_DIMENSIONS
	if (r_dpool) {
		*r_dpool = BLI_mempool_create(
				dim_size, allocsize->totdim,
				bm_mesh_chunksize_default.totdim, BLI_MEMPOOL_ALLOW_ITER);
	}
#endif
#ifdef WITH_MECHANICAL_MESH_REFERENCE_OBJECTS
	if (r_ppool) {
		*r_ppool = BLI_mempool_create(
				ref_size, allocsize->totdim,
				bm_mesh_chunksize_default.totplane, BLI_MEMPOOL_ALLOW_ITER);
	}
#endif
#ifdef WITH_MECHANICAL_GEOMETRY
		if (r_gpool) {
		*r_gpool = BLI_mempool_create(
				geom_size, allocsize->totgeom,
...
static void bm_mempool_init(BMesh *bm, const BMAllocTemplate *allocsize, const bool use_toolflags)
{
	bm_mempool_init_ex(
			...
			&bm->vpool, &bm->epool, &bm->lpool, &bm->fpool, &bm->dpool, &bm->ppool, &bm->gpool);
...
void BM_mesh_toolflags_set(BMesh *bm, bool use_toolflags)
{
	...
	BLI_mempool *dpool_dst = NULL;
	BLI_mempool *ppool_dst = NULL;
	BLI_mempool *gpool_dst = NULL;

	bm_mempool_init_ex(
	        &allocsize, use_toolflags,
			&vpool_dst, &epool_dst, NULL, &fpool_dst, &dpool_dst, &ppool_dst, &gpool_dst);
	...
	BM_mesh_rebuild(
			...
			vpool_dst, epool_dst, NULL, fpool_dst, dpool_dst, ppool_dst);
