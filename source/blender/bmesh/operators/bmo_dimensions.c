		...
		if (type == DIM_TYPE_LINEAR) {
			for (egm = BMO_iter_new(&siter, op->slots_in, "geom", BM_GEOMETRY); egm; egm = BMO_iter_step(&siter)) {
				if (egm->geometry_type == BM_GEOMETRY_TYPE_LINE)
				{
					...
					v_arr[0] = egm->v[0];
					v_arr[1] = egm->v[egm->totverts-1];
					d = BM_dim_create(bm, v_arr, 2, type, NULL, BM_CREATE_NOP, mdm);
			...
			for (egm = BMO_iter_new(&siter, op->slots_in, "geom", BM_GEOMETRY); egm; egm = BMO_iter_step(&siter)) {
				if (ELEM(egm->geometry_type, BM_GEOMETRY_TYPE_ARC, BM_GEOMETRY_TYPE_CIRCLE)) {
					d = BM_dim_create(bm, egm->v, egm->totverts, type, NULL, BM_CREATE_NOP, mdm);
					BMO_dim_flag_enable(bm, d, EXT_KEEP);
				}
			}
			...
			if (v_arr) {
				d = BM_dim_create(bm, v_arr, 3, type, NULL, BM_CREATE_NOP, mdm);
				BMO_dim_flag_enable(bm, d, EXT_KEEP);
				MEM_freeN (v_arr);
			}
		}
			...
			if (v_arr) {
				d = BM_dim_create(bm, v_arr, 4, type, NULL, BM_CREATE_NOP, mdm);
				BMO_dim_flag_enable(bm, d, EXT_KEEP);
				MEM_freeN (v_arr);
			}
		}
